...
Les jeux d’entraînement et de test sont déjà prêt, on créé le jeu de validation à partir du jeu d’entraînement.
```{r}
...
<br> </br> 
Entrainement du modèle sur le jeu d’entraînement
```{r}
history <- model %>% 
  fit(
...
<br> </br> 
Affichage graphique des améliorations de l’entraînement
```{r}
plot(history)
```
...
```
```{r}
predictions = matrix(FALSE, nr = nrow(test), nc = 1)
